...
if ( isset($_GET['id']) && !empty($_GET['id'])  ) {
  ...
  if (preg_match($uuid_pattern, $id)) {
    $remove_varos = remove_varos_check($id, $userip);
    if (is_array($remove_varos["errors"]) && count($remove_varos["errors"]) != 0) {
      $errors = array_unique($remove_varos["errors"]);
      foreach ($remove_varos["errors"] as $key => $err_value) {
      ...
      echo "Check removed. You will no longer receive notifications on maintenance events for this city.<br>";
      echo "</div>";
    }
  } else {
      echo "<div class='alert alert-danger' role='alert'>";;
